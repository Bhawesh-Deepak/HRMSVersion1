...
{
    public class ReadCTCStructureExcelHelper
    {
        public EmployeeSalaryVm GetCTCStructureDetails(IFormFile inputFile)
        {
            ...
            for (int i = 1; i < dataResult.dtResult.Rows.Count; i++)
            {
                ...
                empSalaryModel.CTC = dataResult.dtResult.Rows[i][1].ToString().GetDefaultDBNull<decimal>();
                empSalaryModel.UpdatedDate = DateTime.Now;
                salaryModels.Add(empSalaryModel);

                List<int> ComponentID = new List<int>();
                List<decimal> ComponentValue = new List<decimal>();
                //-------------Start Earning------------------------------------------------
                ComponentValue.Add(dataResult.dtResult.Rows[i][2].ToString().GetDefaultDBNull<decimal>());
                ComponentID.Add(1);
                ComponentValue.Add(dataResult.dtResult.Rows[i][2].ToString().GetDefaultDBNull<decimal>());
                ComponentID.Add(2);
