...
function PopWin(mFn, mw, mh) {
    if (mw == 0)
        mw = screen.width - 50;
    if (mh == 0)
        mh = screen.height * .75;
    var mx = screen.width - mw;
    var my = screen.height - mh;
    mx = mx / 2;
    my = my / 2;

    var mWinName = 'Pop' + parseInt(Math.random() * 100000);
    var mSetting = 'scrollbars=yes,resizable=yes,width=' + mw + ',height=' + mh + ',left=' + mx + ',top=' + my

    popUp = window.open(mFn, mWinName, mSetting);
}

function CloseWindow() {
    if (confirm("Ready to Close Window..")) {
        self.close();
    }
}
