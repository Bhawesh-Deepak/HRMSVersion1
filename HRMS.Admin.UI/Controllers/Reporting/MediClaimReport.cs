...
namespace HRMS.Admin.UI.Controllers.Reporting
{
    public class MediClaimReport : Controller
    {
        ...
        public MediClaimReport(IDapperRepository<MediClaimSqlParams> mediClaimRepository, IHostingEnvironment hostingEnvironment, IGenericRepository<AssesmentYear, int> assesmentyearRepo)
        {
        ...
        public async Task<IActionResult> Index()
        {
            try
            {
                ...
                return await Task.Run(() => View(ViewHelper.GetViewPathDetails("MediClaimReport", "_MediClaimReportIndex")));
            }
            catch (Exception ex)
            {
                string template = $"Controller name {nameof(MediClaimReport)} action name {nameof(Index)} exception is {ex.Message}";
        ...
        public async Task<IActionResult> DownloadMediClaimReport(MediClaimSqlParams model, IFormFile UploadFile)
        {
            ...
            var sFileName = @"MediclaimReport.xlsx";
            ...
            ExcelWorksheet Sheets = Eps.Workbook.Worksheets.Add("Mediclaim");
            ...
            Sheets.Cells["E1"].Value = "Medicalim Amount";
